...
| AI reasoning | Google Gemini API (`gemini-2.5-flash-lite`) |
...
1. Create two bug sessions with related root causes (e.g. two different endpoints both failing due to a database connection pool issue)
2. Log evidence and a hypothesis on each
3. Ask a question in "Ask BugRecall" on one of them — the answer should reference the specific evidence/hypotheses just logged
4. Reopen the session — a "Welcome back" recap banner should appear automatically
5. Check the "Similar Past Cases" panel — it should surface the other related session with a specific reason, not a generic one
...
- [x] Session Resume auto-summary
- [x] Similar Bug Search
